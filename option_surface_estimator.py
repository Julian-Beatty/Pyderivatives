...
from evolutionary_mixture_yi_2024 import (  # noqa: F401
    MixtureSpec,
    fit_mixture_to_calls,
    evolutionary_lwm_fit,
)

from Hermite_Expansion_Jarrow_Rudd_1982 import *  # expects HermiteRNDRowModel
...
class SurfaceConfig:
    ...
    apply_right_safety_clip: bool = False
    right_safety_clip_jump_factor: float = np.e
    # Guardrails for fixed grids
    fixed_grid_sort_unique: bool = True
    fixed_strike_floor: float = 1e-8 
    fixed_moneyness: Optional[np.ndarray] = None  # e.g. np.linspace(0.5, 1.5, 100)

    fixed_strike: Optional[np.ndarray] = None
    fixed_maturity: Optional[np.ndarray] = None
...
class CallSurfaceEstimator:
    ...
    def _build_T_interp_grid(self) -> np.ndarray:
        ...
        # If user provides fixed_maturity, use it as the evaluation grid
        if cfg.fixed_maturity is not None:
            T = np.asarray(cfg.fixed_maturity, float).ravel()
            T = T[np.isfinite(T)]
            if T.size < 2:
                raise ValueError("fixed_maturity must contain at least 2 finite values.")
            if bool(getattr(cfg, "fixed_grid_sort_unique", True)):
                T = np.unique(np.sort(T))
            self.T_interp = T
            return self.T_interp
    
        # Otherwise keep your original behavior
        if not cfg.use_maturity_interp:
            self.T_interp = T_orig.copy()
            return self.T_interp
    
        T_min = float(T_orig.min())
        T_max = float(T_orig.max())
        dt_years = float(cfg.day_step) / 365.0
    ...
    def _set_fixed_eval_grids_if_provided(self):
        """
        If config.fixed_strike / config.fixed_maturity are provided, force the
        evaluation grids (self.strikes, self.K_interp, self.T_interp) to those values.

        Intended for global models. Row-model pipeline continues to use its own
        stage1/stage2/stage3 logic (unless you explicitly route it here).
        """
        fs = getattr(self.config, "fixed_strike", None)
        ft = getattr(self.config, "fixed_maturity", None)

        if fs is not None:
            fs = np.asarray(fs, float).ravel()
            fs = np.unique(fs)
            fs = fs[np.isfinite(fs)]
            fs = fs[fs > 0]
            if fs.size < 5:
                raise ValueError("fixed_strike must contain at least 5 positive finite strikes.")
            self.strikes = np.sort(fs)
            self.K_interp = self.strikes.copy()

        if ft is not None:
            ft = np.asarray(ft, float).ravel()
            ft = np.unique(ft)
            ft = ft[np.isfinite(ft)]
            # If user passed days, they can pass days/365 themselves; we won't guess units here.
            if ft.size < 2:
                raise ValueError("fixed_maturity must contain at least 2 finite maturities.")
            self.T_interp = np.sort(ft)

    def _has_fixed_eval_grids(self) -> bool:
        return (getattr(self.config, "fixed_strike", None) is not None) or (
            getattr(self.config, "fixed_maturity", None) is not None
        )
    ...
    def _build_dense_strike_grid(self) -> np.ndarray:
        ...
        # ---- Priority: fixed_strike -> fixed_moneyness -> default dense+extension ----
        if cfg.fixed_strike is not None and cfg.fixed_moneyness is not None:
            raise ValueError("Provide only ONE of fixed_strike or fixed_moneyness (not both).")
    
        if cfg.fixed_strike is not None:
            K = np.asarray(cfg.fixed_strike, float).ravel()
    
        elif cfg.fixed_moneyness is not None:
            m = np.asarray(cfg.fixed_moneyness, float).ravel()
            if np.any(~np.isfinite(m)):
                raise ValueError("fixed_moneyness contains NaN/inf.")
            if np.any(m <= 0):
                raise ValueError("fixed_moneyness must be strictly > 0 (since K = m*S0).")
            K = m * float(self.S0)
    
        else:
            # Default behavior (your existing dense grid + extension)
            nK_obs = self.strikes_obs.size
            nK_fine = max(cfg.fine_strike_factor * nK_obs, nK_obs)
    
            low = float(self.strikes_obs.min())
            high = float(self.strikes_obs.max())
            ext = max(cfg.strike_extension, 0.0)
            ...
            K = np.linspace(K_min_ext, K_max_ext, nK_fine)
    
        # ---- Guardrails ----
        K = np.where(np.isfinite(K), K, np.nan)
        K = K[np.isfinite(K)]
        if K.size < 3:
            raise ValueError("Strike grid ended up too small after cleaning.")
    
        K = np.maximum(K, float(cfg.fixed_strike_floor))
    
        if bool(cfg.fixed_grid_sort_unique):
            K = np.unique(np.sort(K))
    
        self.strikes = K
        self.K_interp = K.copy()
        return K
    ...
    # ----------------------------
    def _compute_atm_vol(self, k_nearest: int = 3) -> Dict[str, float]:
        """
        ATM vol (robust): median IV of the k strikes nearest spot, pooled across all maturities.
    
        Returns:
          {
            "atm_vol": median pooled IV,
            "atm_K":   representative strike (nearest to spot),
            "atm_T":   NaN (not a single maturity),
            "atm_n":   number of IV points used in the median
          }
        """
        if self.iv_surface is None or self.strikes is None or self.T_interp is None:
            return {"atm_vol": np.nan, "atm_K": np.nan, "atm_T": np.nan, "atm_n": 0}
    
        K = np.asarray(self.strikes, float)
        T = np.asarray(self.T_interp, float)
        iv = np.asarray(self.iv_surface, float)
    
        if iv.shape != (T.size, K.size) or K.size == 0 or T.size == 0:
            return {"atm_vol": np.nan, "atm_K": np.nan, "atm_T": np.nan, "atm_n": 0}
    
        # choose k nearest strikes to S0
        d = np.abs(K - float(self.S0))
        k = int(max(1, min(k_nearest, K.size)))
        idxK = np.argsort(d)[:k]  # indices of nearest strikes
    
        # pool IVs across all maturities for those strikes
        vals = iv[:, idxK].ravel()
        vals = vals[np.isfinite(vals)]
    
        if vals.size == 0:
            return {"atm_vol": np.nan, "atm_K": float(K[int(np.argmin(d))]), "atm_T": np.nan, "atm_n": 0}
    
        atm_vol = float(np.median(vals))
    
        # representative strike: closest to spot
        j0 = int(np.argmin(d))
        return {
            "atm_vol": atm_vol,
            "atm_K": float(K[j0]),
            "atm_T": np.nan,      # not tied to one maturity anymore
            "atm_n": int(vals.size),
        }
    ...
    def _clip_tails_if_exp_jump(
        ...
        *,
        ...
        min_floor: float = 1e-30,
    ):
        """
        Two-sided spike guardrail on q(K).
    
        Left side (center -> left):
          if q[j] / q[j+1] > left_jump at any step, set q[:j+1] = eps
    
        Right side (center -> right):
          if q[j] / q[j-1] > right_jump at any step, set q[j:] = eps
    
        Returns (q_clipped, info_dict)
        """
    ...
    def compute_rnd_surface(
        ...
        eps: float = 1e-12,
        *,
        apply_safety_clip: Optional[bool] = None,        # LEFT tail
        jump_factor: Optional[float] = None,             # LEFT jump threshold
        center: Optional[str] = None,                    # "spot" | "meanS" | "mode"
        apply_right_safety_clip: Optional[bool] = None,  # RIGHT tail
        right_jump_factor: Optional[float] = None,       # RIGHT jump threshold
    ) -> np.ndarray:
        """
        ...
        Post-processing:
          ...
          - Optional left-tail safety clip (center -> left)
          - Optional right-tail safety clip (center -> right)
    
        Notes:
          - The clip is a *spike guardrail*: if q jumps up by more than the specified
            multiplicative factor in a single step, we clip the rest of that tail to eps.
          - Left clip is typically safer; right clip should often use a larger threshold.
        """
        ...
        if apply_right_safety_clip is None:
            apply_right_safety_clip = bool(getattr(self.config, "apply_right_safety_clip", False))
        if right_jump_factor is None:
            right_jump_factor = float(getattr(self.config, "right_safety_clip_jump_factor", jump_factor))
    
        rnd = np.empty_like(C)
    
        # diagnostics
        self.left_jump_clip_info = []   # now stores BOTH tail info dicts per T
        self.safety_clip_status = "Unused"
    
        for i, T in enumerate(T_grid):
            # second derivative in strike
            ...
            # optional tail spike guardrails (left and/or right)
            if apply_safety_clip or apply_right_safety_clip:
                qi, info = self._clip_tails_if_exp_jump(
                    qi, K, eps,
                    center=center,
                    left_jump=float(jump_factor),
                    right_jump=float(right_jump_factor),
                    clip_left=bool(apply_safety_clip),
                    clip_right=bool(apply_right_safety_clip),
                )
                self.left_jump_clip_info.append({"T": float(T), **info})
                if info.get("clipped_left", False) or info.get("clipped_right", False):
                    self.safety_clip_status = "Used"
            else:
                self.left_jump_clip_info.append({
                    "T": float(T),
                    "clipped_left": False, "left_cut_idx": None, "left_K_cut": None, "left_ratio": None,
                    "clipped_right": False, "right_cut_idx": None, "right_K_cut": None, "right_ratio": None,
                    "center": center,
...
if __name__ == "__main__":
    ...
    cfg = SurfaceConfig(
        ...
        apply_right_safety_clip=False,           # right tail
        right_safety_clip_jump_factor=np.e,
        fixed_moneyness=np.linspace(0.05, 2.2, 100),
        fixed_maturity=np.linspace(70/365, 365/365, 365),
    )

    # For demo purposes, just use row mixture by default with safety clip OFF:
    # cfg = SurfaceConfig(
    #     row_model=RowModelConfig(model="mixture_evolutionary",M_max=1,),
    #     use_maturity_interp=False,
    #     day_step=14,
    ...
    # cfg = SurfaceConfig(
    #     row_model=RowModelConfig(model="hermite",),
    #     use_maturity_interp=False,
    #     day_step=14,
    #     strike_extension=0.8,
    #     fine_strike_factor=3,
...
def load_and_combine_pickles_cwd(
    filename_prefix: str,
    filename_suffix: str = ".pkl",
    sort_keys: bool = True,
) -> Dict[Any, Any]:
    """
    Load and merge multiple pickle dictionary files from the current working directory.

    Parameters
    ----------
    filename_prefix : str
        Common filename prefix
        (e.g. "brazil_information_dict_demo_part").
    filename_suffix : str, default=".pkl"
        File extension.
    sort_keys : bool, default=True
        Sort keys after merging (recommended for time series).
    ...
    files = sorted(cwd.glob(f"{filename_prefix}*{filename_suffix}"))

    if not files:
        raise FileNotFoundError(
            f"No files found in {cwd} matching '{filename_prefix}*{filename_suffix}'"
        )
    ...
    for file in files:
        ...
        if not isinstance(part, dict):
            raise TypeError(f"{file.name} does not contain a dictionary.")

        # Safety: prevent accidental overwrites
        overlap = combined.keys() & part.keys()
        if overlap:
            raise ValueError(
                f"Duplicate keys detected in {file.name}: "
                f"{list(overlap)[:5]}"
            )

        combined.update(part)
        print(f"Loaded {len(part)} entries from {file.name}")

    if sort_keys:
        combined = dict(sorted(combined.items()))

    print(f"\nCombined dictionary size: {len(combined)}")
    print(f"Loaded from directory: {cwd}")

    return combined

def split_dict_into_pickles(
    data: Dict[Any, Any],
    output_dir: str,
    base_filename: str = "information_part",
    n_splits: int = 4,
    sort_keys: bool = True,
):
    """
    Split a dictionary into `n_splits` smaller dictionaries and save each as a pickle.

    Parameters
    ----------
    data : dict
        Original dictionary (e.g. information_dict).
    output_dir : str
        Directory where pickle files will be saved.
    base_filename : str, default="information_part"
        Base name for pickle files.
    n_splits : int, default=4
        Number of chunks.
    sort_keys : bool, default=True
        Whether to sort keys before splitting (recommended for time series).
    """
    ...
    # Compute split sizes (handles non-divisible lengths)
    split_sizes = [(n + i) // n_splits for i in range(n_splits)]

    idx = 0
    for i, size in enumerate(split_sizes, start=1):
        chunk_keys = keys[idx : idx + size]
        chunk_dict = {k: data[k] for k in chunk_keys}
        idx += size

        filename = Path(output_dir) / f"{base_filename}_{i}.pkl"
        with open(filename, "wb") as f:
            pickle.dump(chunk_dict, f)

        print(f"Saved {len(chunk_dict)} entries → {filename}")

#split_dict_into_pickles(information_dict,output_dir="information_splits", base_filename="brazil_information_dict_demo_part",n_splits=5)
