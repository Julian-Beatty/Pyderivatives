...
class CallSurfaceEstimator:
    ...
    def plot_rnd_surface(self, title: str = "Risk-neutral density surface"):
        """3D colored hillshade plot of RND surface (with visible z-label)."""
        import numpy as np
        import matplotlib.pyplot as plt
        from mpl_toolkits.mplot3d import Axes3D  # noqa: F401
        ...
        # ---- Z label: put it in 2D axes coords so it never gets clipped ----
        ax.set_zlabel("")  # avoid a clipped default zlabel
        ax.text2D(0.98, 0.52, "f(k)", transform=ax.transAxes,
                  rotation=90, va="center", ha="right")
    
        # Optional: improve readability of z ticks
