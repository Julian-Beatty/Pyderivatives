...
from typing import List, Literal, Tuple, Iterable, Optional
...
from Heston_model import *  # assumes heston_call_prices_fast, vector_implied_vols, etc.
...
#      + fixed-mixture shortcut via fixed_M / fixed_M1
# ---------------------------------------------------------------
...
def evolutionary_lwm_fit(
    ...
    if fixed_M is not None:
        ...
        if metric == "loss":
            ...
            # metric='density': nothing to compare against for selection,
            # but we can still compute MSE as a diagnostic if desired.
    ...
    for M in range(1, M_max + 1):
        ...
        if use_wald:
            wald_stat, p_val = wald_spec_test_mixture(
                K=K,
                C_mkt=C_mkt,
                S0=S0,
                r=r,
            ...
            # print(f"M={M}, M1={M1_M}, Wald={wald_stat:.3f}, p={p_val:.3f}")
            if p_val > wald_alpha:
...
if __name__ == "__main__":
    ...
    x_grid = np.linspace(1e-6, 300, 200)
