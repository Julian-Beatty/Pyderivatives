...
def noisy_data_function(k, true_calls, true_s, noise_dict):
    ...
    if mode == "static":
        rng = np.random.default_rng(seed)            # or remove seed for non-reproducible
        eps = rng.uniform(0, 0.01, size=np.shape(C))
        noisy = C * (1.0 + eps)
    ...
    if mode=="sait":
        ##implement the noise specification based on liquidity factor, moneyness, and percent of options strike price.
        scale=float(noise_dict.get('scale',0.05)) #assume scale of 5% if not included
        
        
        min_spread = 0.00036 * true_s      # 0.0036% of underlying
        max_spread = 0.0014   * true_s 

        moneyness=k/true_s
        
        liquidity_factor=1+(2/0.20)*abs(moneyness-1)
        raw_spread=scale*C ##X% of thee option's value
        spread=np.clip(raw_spread,min_spread,max_spread)
        high = 0.5 * spread * liquidity_factor
        eps = rng.uniform(low=0.0, high=high)
        
        #eps = rng.uniform(low=0.0, high=0.5*spread)*liquidity_factor

        noisy = C + eps
    if mode=="bondarenko":
        scale=float(noise_dict.get('scale',0.5))
        s_half  = relevant_spread_si(true_calls, k, true_s, c=scale)  # spreads = 0.5 * min(...)
        s = np.asarray(s_half, dtype=float)

        eps = rng.uniform(low=-s, high=s)       # elementwise: U[-s_i, s_i]
        noisy = C + eps
...
def mean_square_error(true_rnd,estimated_rnd):
    ...
    """
    mse=np.mean((true_rnd-estimated_rnd)**2)
    return mse
