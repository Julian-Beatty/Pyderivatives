...
def breeden_litzenberger_pdf(
    ...
    *,
    ...
    Z = np.trapezoid(q, K_grid, axis=1).reshape(-1, 1)
    q = q / Z
    return q
...
def _clip_tails_if_exp_jump_row(
    ...
    *,
    ...
    if center == "spot":
        ...
        area = float(np.trapezoid(qi, K))
        if not (np.isfinite(area) and area > 0):
            return qi, info
        qn = qi / area
        meanS = float(np.trapezoid(K * qn, K))
...
def strike_rnd_to_return_density(
    ...
    *,
    ...
    if normalize:
        if q_ret.ndim == 1:
            Z = np.trapezoid(q_ret, grid)
            q_ret = q_ret / Z
        else:
            Z = np.trapezoid(q_ret, grid, axis=1).reshape(-1, 1)
            q_ret = q_ret / Z
